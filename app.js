...
mongoose.connect("mongodb+srv://admin-jake:139489@cluster0.ap4pl.mongodb.net/todoListDB?retryWrites=true&w=majority", {
  ...
  List.findOne({
    ...
    if (err) {
      ...
      if (foundList) {
        res.render("list", {
          todayIs: today,
          userNameIs: foundList.userName,
          weatherIs: foundList.weather,
...
app.post("/", function(req, res) {
  let userNameForFunction = req.body.userNameInput;
    ...
    response.on("data", function(data) {
      ...
      if (typeof weatherObject.weather === "undefined") { //if user entered invalid city name
        ...
        if (cleanUserNameForFunction === "home") {
          ...
          List.findOneAndUpdate({
            userName: cleanUserNameForFunction
          }, {
            $set: {
              weather: weatherForFunc
            }
          }, function(err, foundList) {
            if (!err) {
